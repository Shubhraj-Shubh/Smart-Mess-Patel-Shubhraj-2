...
import { UpdateMenu } from "@/actions/MessMenuActions";
...
type MenuType = {
  ...
  menu: {
    day:
      ...
      | "Sunday";
    meals: {
      morning: { veg: string[]; nonVeg: string[] };
      lunch: { veg: string[]; nonVeg: string[] };
      eveningSnack: { veg: string[]; nonVeg: string[] };
      dinner: { veg: string[]; nonVeg: string[] };
    };
  }[];
};

export default function MessMenuForm({ menu }: { menu: MenuType }) {
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      menu: daysOfWeek.map((day, ind) => ({
        day: day,
        meals: {
          morning: {
            veg: menu.menu[ind].meals.morning.veg,
            nonVeg: menu.menu[ind].meals.morning.nonVeg,
          },
          lunch: {
            veg: menu.menu[ind].meals.lunch.veg,
            nonVeg: menu.menu[ind].meals.lunch.nonVeg,
          },
          eveningSnack: {
            veg: menu.menu[ind].meals.eveningSnack.veg,
            nonVeg: menu.menu[ind].meals.eveningSnack.nonVeg,
          },
          dinner: {
            veg: menu.menu[ind].meals.dinner.veg,
            nonVeg: menu.menu[ind].meals.dinner.nonVeg,
          },
        },
      })),
    },
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      const res = await UpdateMenu(menu._id, values);
    ...
    <div className="container py-10 mx-auto">
      <Card className="w-full">
        <CardHeader>
          ...
          <CardDescription>
            Update a seasonal menu for your mess. Configure meals for each day
            of the week.
          </CardDescription>
        </CardHeader>
        <CardContent>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-col">
                      ...
                      <FormDescription>When this menu starts.</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                        ...
                        <Card key={mealTime}>
                          ...
                          <CardContent className="space-y-4">
                            ...
                            <div className="space-y-2">
                              ...
                              {form
                                .watch(`menu.${dayIndex}.meals.${mealTime}.veg`)
                                .map((_, vegIndex) => (
                                  <div
                                    key={vegIndex}
                              ...
                              >
                                <Plus className="h-4 w-4 mr-2" /> Add Veg Option
                              </Button>
                            </div>
